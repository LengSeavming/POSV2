...
export class PetTypeService {
  ...
  async getData(): Promise<{
    ...
    const data = await PetType.findAll({
      attributes: [
        ...
        "image",
        "created_at",
        [Sequelize.fn("COUNT", Sequelize.col("pets.id")), "n_of_pets"], // Fixing the COUNT function
      ],
      include: [
        {
          model: Pet,
    ...
    const dataFormat = data.map((type) => ({
      ...
      n_of_pets: type.get("n_of_pets") || 0, // Using `get` method to access the alias field
    }));
  ...
  async create(
    ...
    const dataFormat = {
      data: productType,
      message: "Pet type has been created.",
