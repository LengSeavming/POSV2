...
import { CreateOrderDto, CreatePetOrderDto } from './order.pos.dto';
...
export class OrderService {
    ...
    async getPet(): Promise<{ data: { id: number, name: string, pets: Pet[] }[] }> {
        const data = await PetType.findAll({
            ...
            include: [
                {
                    model: Pet,
                    as: 'pets',  // Add this line to specify the association name
                    attributes: ['id', 'type_id', 'name', 'image', 'unit_price', 'code'],
                    include: [
                        {
                            model: PetType,
        ...
        const petdataFormat: { id: number, name: string, pets: Pet[] }[] = data.map(type => ({
            id: type.id,
            name: type.name,
            pets: (type as any).pets || []
        }));

        return { data: petdataFormat };
    }

    // Method for creating an order
    async petOrder(cashierId: number, orderDto: CreatePetOrderDto): Promise<{ data: Order, message: string }> {
        // Initializing DB Connection
        const sequelize = new Sequelize(sequelizeConfig);
        let transaction: Transaction;
        ...
        try {
            ...
            // Find Total Price & Order Details
            let totalPrice = 0;
            const cartItems = JSON.parse(orderDto.petcart); // Assuming cart is a JSON string

            // Loop through cart items and calculate total price
            for (const [petId, qty] of Object.entries(cartItems)) {
                const pet = await Pet.findByPk(parseInt(petId)); // Find pet by its ID
                ...
                if (pet) {
                    ...
                    totalPrice += Number(qty) * pet.unit_price; // Add to total price
                }
            }
    ...
    // Private method to generate a unique receipt number
    private async _petgenerateReceiptNumber(): Promise<number> {
        const number = Math.floor(Math.random() * 9000000) + 1000000;
        return await Order.findOne({
            where: {
                receipt_number: number,
            ...
            if (order) {
                return this._petgenerateReceiptNumber();
            }
            return number;
        });
    }
