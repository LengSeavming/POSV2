...
import { PetReport, ProductReport } from "./interface";
...
export class ReportService {
  ...
  async generateCashierReportBaseOnStartDateAndEndDate(
    ...
    return this.generateAndSendReport(
      reportData,
      process.env.JS_TEMPLATE_CASHIER,
      "Cashier Sales Report",
      "របាយការណ៍លក់តាមអ្នកគិតប្រាក់"
    );
  }
  ...
  async generatePetReportBaseOnStartDateAndEndDate(
    startDate: string,
    endDate: string,
    userId: number
  ) {
    ...
    const user = await this.fetchUser(userId);
    const products = await this.fetchProducts(start, end);
    const pets = await this.fetchPets(start, end);

    const petData = this.processPetData(pets);
    const productData = this.processProductData(products);
    const totalSales = this.calculateTotal(productData, "total_sales");
    const totalQty = this.calculateTotal(productData, "total_qty");
    ...
    return this.generateAndSendReport(
      reportData,
      process.env.JS_TEMPLATE_PET,
      "Product Sales Report",
      "របាយការណ៍លក់តាមសត្វចញ្ចឹម"
    );
  }
  ...
  private async fetchPets(startDate: Date, endDate: Date) {
    return Pet.findAll({
      attributes: ["id", "name", "unit_price"],
      include: [
        { model: PetType, as: "type", attributes: ["id", "name"] },
        {
          model: OrderDetails,
          as: "pod",
          where: { created_at: { [Op.between]: [startDate, endDate] } },
  ...
  private processPetData(pets: Pet[]): PetReport[] {
    return pets.map((pet) => {
      const totalQty = pet.pod.reduce((sum, detail) => sum + detail.qty, 0);
      const totalSales = totalQty * pet.unit_price;
